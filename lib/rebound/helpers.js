...
function _each(value, options){
  ...
  _.each(value, function(obj, key, list){
    ...
    // Lazy value attaches listener to the object we are iterating over. Returns the dom for this list element.
    var lazyValue = new LazyValue(function(){
    ...
    // When we are notified of the data object changing, update the dom.
    lazyValue.onNotify(function(lazyValue) {
      options.placeholder.update(lazyValue.value());
    });
    ...
    options.placeholder.append(lazyValue.value());

    // TODO: Needs cases for adding and removing elements. It shouldnt re-render whole list.
