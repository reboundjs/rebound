...
var AppLoader = Backbone.AppLoader = (function (Backbone) {
  ...
  function installPageResources(PageApp, primaryRoute, isGlobal) {
    ...
    if(!isGlobal && pageApp){
      ...
      _.each(pageApp.routes, function (value, key) {
        ...
        Backbone.history.handlers = _.filter(Backbone.history.handlers, function(obj){return obj.route.toString() !== appRouter.__routeToRegExp(key).toString();})
    ...
    _.each(pageInstance.routes, function (value, key) {
      ...
      appRouter.route(key, value.__name, appRouter[routeFunctionName]);
    });
  ...
  function loadPageResources(primaryRoute, isGlobal) {
    ...
    var jsUrl = config.jsPrefix + primaryRoute + config.jsSuffix,
  ...
  _.extend(AppLoader.prototype, Backbone.Events, {
    ...
    start: function() {
      ...
      _.each(config.globalControllers, function(route){
        loadPageResources(route, true);
      })
      ...
      Backbone.Controller.prototype.router = this;
    }
  });
