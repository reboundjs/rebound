...
function __if(params, hash, options, env){
  ...
  if(Backbone && condition instanceof Backbone.Model || condition instanceof Backbone.Collection){
    console.log('ERR: if statement argument must be a primitive.');
    return;
  }
...
function streamFor(context, path, morph) {
...
function streamifyArgs(context, params, options, helpers) {
  ...
  for (var i = 0, l = params.length; i < l; i++) {
    if (options.types[i] === 'id') {
      params[i] = streamFor(context, params[i], morph);
    }
  }
  ...
  for (var key in hash) {
    if (hashTypes[key] === 'id') {
      hash[key] = streamFor(context, hash[key], morph);
    }
  }
}
    ...
    for (var i = 0; i < len; i++) {
      if(_.isString(helper.__params[i])){
        ...
        if(helper.__params[i].indexOf('@each') > 0){
          ...
          addObserver(get(options.context, tmp[0]), '@each.' + tmp[1], options.lazyValue, el);

          // Listen to collection changes (add, remove, reset, etc)
          addObserver(options.context, tmp[0], options.lazyValue, el);
        }
        else{
          addObserver(context, helper.__params[i], options.lazyValue, el);
        }
      }
    }
  }
...
hooks.content = function(placeholder, path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, placeholder);
  }
...
hooks.element = function(element, path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, element);
  }
...
hooks.subexpr = function(path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, (options || true));
  }
...
export default { registerHelper: registerHelper, registerPartial: registerPartial, hooks: hooks, helpers: helpers }
