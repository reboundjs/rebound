...
// import { notify } from "rebound/runtime";
...
function __on(params, hash, options, env){
  ...
  for(i = 1; i<len; i++){
    ...
    $(env.dom.document).on(params[0], delegate, data, function(event){
      return helpers.__callOnController(callback, event)
    })
    ...
    if(!options.lazyValue.eventsBound){
      ...
      if (attr) {
        $(options.element).on('keyup', options.context, function(event){
          if(event.keyCode == 13)
            helpers.__callOnController(attr, event);
        });
      }

      // On key up, update our bound data value
      $(options.element).on('keyup', options.context, function(){
...
function __if(params, hash, options, env){
  ...
  if(Backbone && condition instanceof Backbone.Model){
    condition = true;
  }

  // If our condition is an array, handle properly
  if(_.isArray(condition) || Backbone && condition instanceof Backbone.Collection){
...
function isComputedProperty(model, path){
  var parentPath = _.compact(path.split(/(?:\.|\[|\])+/)),
      child = parentPath.pop(),
      val;
  parentPath = parentPath.join('.');
  val = get(model, parentPath);
  val = (val instanceof Backbone.Collection)? val.models[child] : ((model instanceof Backbone.Model)? val.attributes[child] : val[child]) || false;
  return (_.isFunction(val)) ? val : false;

}

// Add a callback to a given model to trigger when its value at 'path' changes.
function addObserver(model, path, lazyValue, morph) {
  var length;
// console.log("OBS", model, path, lazyValue, morph)
  ...
  length = model.__observers[path].length;

  model.__observers[path].push(function() {
    try{
      // If morph element is no longer on the page, remove our bound observer. Otherwise, notify our lazy value of the value change.
      // TODO: document.contains is probably very inefficient. Find some other way to track if a morph is still on the page.
      return (morph.element && !document.contains(morph.element)) ? delete model.__observers[path][length] : lazyValue.notify();
    } catch(err) {
      console.log("ERROR: Memory Leak With " + path + " On ", model, "Calling", model.__observers[path][length]);
      console.log(err);
    }
...
function bindParams(helper, options, morph){
  // If we have custom defined databinding, bind to those vars.
  if(_.isArray(helper.__params)){
    console.log(helper, helper.__params, options);
    ...
    for (var i = 0; i < len; i++) {
      if(_.isString(helper.__params[i])){
        ...
        if(helper.__params[i].indexOf('@each') > 0){
          var tmp = helper.__params[i].split(/.?@each.?/);
          ...
          console.log('fghjk',tmp[0], tmp[1])
          addObserver(get(options.context, tmp[0]), '@each.' + tmp[1], options.lazyValue, morph);

          // Listen to collection changes (add, remove, reset, etc)
          addObserver(options.context, tmp[0], options.lazyValue, morph);
        }
        else{
          addObserver(options.context, helper.__params[i], options.lazyValue, morph);
        }
      }
    }
  }
...
function streamFor(context, path, morph, options) {
  var lazyValue;
  // If value is a computed property, return the computed property function, else return value.
  var computedProperty = isComputedProperty(context, path);
  if(computedProperty){
    // New lazy value calls each of this computed property's observers
    lazyValue = new LazyValue(function() {
      var observers = (context instanceof Backbone.Collection) ? context.models.__observers : context.__observers;
      console.log("VALUE", observers[path]);
    ...
    console.log('HELPER IS ARG ', computedProperty, context, path, morph, options);
    bindParams(computedProperty, {context: context, lazyValue: lazyValue}, morph);
    return lazyValue;
  }
  ...
  addObserver(context, path, lazyValue, morph);
...
function streamifyArgs(context, params, options, helpers) {
  ...
  for (var i = 0, l = params.length; i < l; i++) {
    if (options.types[i] === 'id') {
      params[i] = streamFor(context, params[i], morph, options);
    }
  }
  ...
  for (var key in hash) {
    if (hashTypes[key] === 'id') {
      hash[key] = streamFor(context, hash[key], morph, options);
    }
  }
}
...
function constructHelper(el, path, context, params, options, env, helper) {
  ...
  bindParams(helper, options, (options.placeholder || options.element));
...
hooks.content = function(placeholder, path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, placeholder, options);
  }
...
hooks.element = function(element, path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, element, options);
  }
...
hooks.webComponent = function(placeholder, path, context, options, env) {
  ...
  // Abstracts our helper to provide a handlebars type interface. Constructs our LazyValue.
  lazyValue = constructHelper(placeholder, path, context, [], options, env, helper)
...
hooks.subexpr = function(path, context, params, options, env) {
  ...
  if (helper) {
    ...
    lazyValue = streamFor(context, path, (options || true), options);
  }
...
function registerTemplate(name, func){
  if(_.isFunction(func) && typeof name === 'string')
    templates[name] = func;
}
