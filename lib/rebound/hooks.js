...
function __attribute(params, hash, options, env) {
  ...
  if(options.element.tagName === 'INPUT' && options.params.length === 2 && key === 'value'){

    builder.onNotify(function(lazyValue) {
      options.element.value = lazyValue.value();
    });

    options.element.value = builder.value();

    var inputKeyUp = function(){
      options.context.set('firstName', this.value)
    }

    $(options.element).on('keyup', inputKeyUp)

    return;
  }

  builder.onNotify(function(lazyValue) {
    options.element.setAttribute(key, lazyValue.value());
  });

  options.element.setAttribute(key, builder.value());
}
...
function constructHelper(el, path, context, params, options, env, helper) {
  ...
  lazyValue = new LazyValue(function() {
    ...
    // len = options.hash.values.length;
    // for(i=0; i<len; i++){
    //   hash.push(( (options.hash.values[i].isLazyValue) ? options.hash.values[i].value() : options.hash.values[i] ));
    // }


    // Call our helper functions with our assembled args.
    return helper.apply(context, [plainParams, hash, options, env]);
  });
