...
function __each(params, hash, options, env){
  ...
  _.each(value, function(obj, key, list){
    ...
    // Even if rendered already, update each element's index in case of order changes or element removals
    obj.set({'@index': key});
...
hooks.webComponent = function(placeholder, path, context, options, env) {
  ...
  // If our controller constructor has not been created yet, create our constructor and cache it. Otherwise, use our cached constructor.
  controller = component.controller = ((!_.isFunction(component.controller)) ? Backbone.Controller.extend(component.controller) : component.controller);
  ...
  console.log("WEB COMPONENT", component, data, controller);
  ...
  lazyValue = new LazyValue(function() {
    ...
    // TODO: Add DOMNodeRemoved callback here to instance.dom. When called, destroy our controller. Only works in ie9+... is there a better way?


    if(instance.readyCallback){
      instance.readyCallback.call(instance);
    }
  ...
  if (lazyValue) {
    ...
    value = lazyValue.value();
    if(value !== undefined) placeholder.append(value);
    if(instance.insertedCallback){
      // Give our dom a second to update and call our inserted callback
      setTimeout(function(){instance.insertedCallback.call(instance);}, 0)
    }
  }
}
