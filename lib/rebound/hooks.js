...
function __if(params, hash, options, env){
  ...
  if(Backbone && params[0] instanceof Backbone.Model || params[0] instanceof Backbone.Collection){
...
function constructHelper(el, path, context, params, options, env, helper) {
  ...
  console.log("Creating Bound Paramater Dependancies", path)
    ...
    for (var i = 0; i < len; i++) {
      if(_.isString(helper.__params[i])){
        ...
        if(helper.__params[i].indexOf('@each') > 0){
          ...
          addObserver(get(options.context, tmp[0]), '@each.' + tmp[1], function() {
            options.lazyValue.notify();
          });
          // Listen to collection changes (add, remove, reset, etc)
          addObserver(options.context, tmp[0], function() {
            options.lazyValue.notify();
          });
        }
        else{
          console.log("NO @EACH", context, helper.__params[i])
          addObserver(context, helper.__params[i], function() {
            options.lazyValue.notify();
          });
        }
      }
