...
if(!window.Backbone.Controller){
  ...
  // We override the _removeReferance function to always trigger a dom sync on removal.
  Backbone.Collection.prototype._remove = Backbone.Collection.prototype.remove;
  Backbone.Collection.prototype.remove = function(models, options){
    ...
    if(!_.isArray(this.models.__removedIndex)) this.models.__removedIndex = [];

    // If a silent remove and this is the first silent remove since the last dom update, save our initial indicies
    if( options.silent && this.models.__removedIndex.length == 0 ){
  ...
  _.extend(Controller.prototype, Backbone.Events, {
    ...
    _on: function(element, name, params, options, helpers){
      var id = _.uniqueId('event');
      $(element).attr('data-event', id);
      var delegate = options.hash.selector || '[data-event='+id+']';
      var data = options.hash.data || {};
      // Make sure we only attach once for each combination of selector and callback
      this.outlet.off(params[0], delegate, this[params[1]]).on(params[0], delegate, data, this[params[1]]);
    },
    ...
    _startListening: function(){
      ...
      this.listenTo(this.data, 'change', this._onChange);
      this.listenTo(this.data, 'add', this._onAdd);
      this.listenTo(this.data, 'remove', this._onRemove);
    ...
    _notify: function(obj, changed){
      ...
      while(obj){
        ...
        if(obj.__observers && paths.length){
          console.log("notifying:", obj, obj.__observers, paths)
          notify(obj, paths);
        }
        obj = obj.__parent;
      }
