...
QUnit.test('Rebound Helpers - Each', function(assert) {
  ...
  // Block Scoped Bound Attributes
    ...
    data = new Model({parent: 'bar', arr: [{val:'foo'}]});
    dom = template.render(data);
    equal(dom.fragment.firstChild.firstChild.className, 'foo', 'Inside each blocks have access to both block scopes for attribute values');
    data.set('arr[0].val', 'bar');
    equal(dom.fragment.firstChild.firstChild.className, 'bar', 'Block scoped values used as attribute values are data bound');
  ...
  // Boolean Block Scoped Bound Attributes
    ...
    dom = template.render(data);
    equal(dom.fragment.firstChild.firstChild.checked, true, 'Inside each blocks have access to both block scopes for boolean attribute values');
    data.set('arr[0].val', false);
    equal(dom.fragment.firstChild.firstChild.checked, false, 'Block scoped values used as boolean attribute values are data bound');
