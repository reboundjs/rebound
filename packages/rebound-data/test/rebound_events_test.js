...
// Deep Set - Primitive Values
  ...
  model.get('a.b[1][0].c').on('change:d', function(model, value, options){
    ...
    equal(model.path, 'a.b[1][0].c', layerID + "Passed model's path is correct." );
    deepEqual(model.changed, {d: 2}, layerID + "Passed model's changedAttributes are correct." );
  ...
  model.get('a.b[1][0]').on('change:c', function(model, value, options){
    ...
    equal(model.path, 'a.b[1][0]', layerID + "Passed model's path is correct." );
    deepEqual(model.changed, {c: {d: 2}}, layerID + "Passed model's changedAttributes are correct." );
  ...
  model.get('a.b[1]').on('change:[0]', function(collection, value, options){
    ...
    equal( collection.path, 'a.b[1]', layerID + "Passed model's path is correct." );
    deepEqual( collection.changed, {0: {c: {d: 2}}}, layerID + "Passed collection's changedAttributes are correct." );
  ...
  model.get('a.b').on('change:[1]', function(collection, value, options){
    ...
    equal( collection.path, 'a.b', layerID + "Passed model's path is correct." );
    deepEqual( collection.changed, {1: {0: {c: {d: 2}}}}, layerID + "Passed collection's changedAttributes are correct." );
  ...
  model.get('a').on('change:b', function(model, value, options){
    ...
    equal(model.path, 'a', layerID + "Passed model's path is correct." );
    deepEqual(model.changed, {b: {1: {0: {c: {d: 2}}}}}, layerID + "Passed collection's changedAttributes are correct." );
  ...
  model.on('change:a', function(model, value, options){
    ...
    equal(model.path, '', layerID + "Passed model's path is correct." );
    deepEqual(model.changed, {a: {b: {1: {0: {c: {d: 2}}}}}}, layerID + "Passed collection's changedAttributes are correct." );
  ...
  // Deep Set - Computed Values
    ...
    model.get('a[0]').on('change:b', function(model, value, options){
      var layerID = "[Layer 2, Path 0] ";
      equal(count++, 0, layerID + "Named change event is triggered in order." );
      equal(model.path, 'a[0]', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {b: 2}, layerID + "Passed model's changedAttributes are correct." );
      equal( value, 2, layerID + "Passed value is correct" );
    });
    ...
    model.get('a').on('change:[0]', function(model, value, options){
      ...
      equal(count++, 2, layerID + "Named change event is triggered in order." );
      equal(model.path, 'a', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {0: {b: 2}}, layerID + "Passed model's changedAttributes are correct." );
    ...
    model.on('change:a', function(model, value, options){
      ...
      equal(count++, 5, layerID + "Named change event is triggered in order." );
      equal(model.path, '', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {a: {0: {b: 2}}}, layerID + "Passed model's changedAttributes are correct." );
      equal( value.cid, model.get('a').cid, layerID + "Passed value is correct" );
    });
    ...
    model.on('change:computedProp', function(model, value, options){
      ...
      equal(count++, 6, layerID + "Named change event is triggered in order." );
      equal(model.path, '', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {a: model.get('a'), "a[0]": model.get('a[0]'), "a[0].b": 2, computedProp: 2}, layerID + "Passed model's changedAttributes are correct." );
      equal( value, 2, layerID + "Passed value is correct" );
      end();
    });
...
debugger;
    model.get('computedModel[0]').on('change:b', function(model, value, options){
      ...
      equal(count++, 7, layerID + "Named change event is triggered in order." );
      equal(model.path, 'computedModel[0]', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {b: 2}, layerID + "Passed model's changedAttributes are correct." );
      equal( value, 2, layerID + "Passed value is correct" );
      end();
    });
    ...
    model.get('computedModel').on('change:[0]', function(model, value, options){
      ...
      equal(count++, 9, layerID + "Named change event is triggered in order." );
      equal(model.path, 'computedModel', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {0: {b: 2}}, layerID + "Passed model's changedAttributes are correct." );
      equal( value.cid, model.at(0).cid, layerID + "Passed value is correct" );
      end();
    });
    ...
    model.on('change:computedModel', function(model, value, options){
      ...
      equal(count++, 12, layerID + "Named change event is triggered in order." );
      equal(model.path, '', layerID + "Passed model's path is correct." );
      deepEqual(model.changed, {a: {0: {b: 2}}, computedModel: {0: {b: 2}}}, layerID + "Passed model's changedAttributes are correct." );
