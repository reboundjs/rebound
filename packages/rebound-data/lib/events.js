...
// Event Package Symbol used to hide event binding metadata from users
const EVENTS = Symbol('Rebound-Events');
...
class EventTracker {
  ...
  // Given an event of the form: `name:optional.path[parts]`, return an array of
  // the path parts, or empty array if the event has no path data.
  static extractPath(str){
    if (typeof str !== 'string') return [];
    return  Path.split(str.split(':')[1] || '');
  }

  // Given an event of the form: `name:optional.path[parts]`, return the name.
  static extractName(str){
    if (typeof str !== 'string') return '';
    return str.split(':')[0] || '';
  }

  // Check if the string passed contains a `@each` or `@all` wildcard part.
  static hasWildcard(name){
    return !!~name.search(/[\.:]@each/) || !!~name.search(/[\.:]@all/);
  }

  // Check if the string passed contains a paths part.
  static hasPath(name){
    return !!~name.indexOf(':');
  }
}
...
function onApi(events, name, callback, options) {
  if (callback) {
    ...
    // TODO: Clean up
    if (EventTracker.hasWildcard(name) && !delegates.hasOwnProperty(name)){
      delegates[name] = EventTracker.extractPath(name);
...
// Delegate match takes two path parts arrays and returns true or false
...
// Handles triggering the appropriate event callbacks. Ony trigger `all` callbacks
...
function triggerApi(events, name, callback, args) {
  if (events) {
    ...
    var eventPath = EventTracker.extractPath(name);
    var eventName = EventTracker.extractName(name);
